...
class Agent:
    ...
    def __init__(
        ...
        self.alpha = alpha
        self.actor1 = FeedForwardNN(6, 1, self.alpha, HP=self.HP, use_optimizer=False)
        self.actor2 = FeedForwardNN(12, 1, self.alpha, HP=self.HP, use_optimizer=False)
        self.combined_optimizer, self.scheduler = init_combined_parameters(
            self.actor1, self.actor2, self.alpha
        )
        self.critic = FeedForwardNN(6, 1, self.alpha, actor=False, HP=self.HP)
        self.memory = PPOMemory(batch_size)
        self.n_epochs = n_epochs

    def remember(self, state, action, probs, vals, reward, done, current_episode):
        self.memory.store_memory(
            state, action, probs, vals, reward, done, current_episode
        )
    ...
    def forward_actor1(self, observation, locations):
        # Prepare batch data
        batch_data = []
    ...
    def forward_actor2(self, observation, locations, idx_first_node):
        # Prepare batch data
        batch_data = []
        for i in range(len(observation)):  # Avoid out-of-bounds indexing
            if (
    ...
    def forward_critic(self, locations, observation, idx_first_node, idx_second_node):
        # Prepare batch data
        batch_data = []
        for i in range(len(observation)):  # Avoid out-of-bounds indexing
            batch_data.append(
    ...
    def choose_action(self, observation, episode):
        ...
        probs1 = self.forward_actor1(observation, locations)
        ...
        probs2 = self.forward_actor2(observation, locations, first_node_idx)
        ...
        value = self.forward_critic(
            locations, observation, first_node_idx, second_node_idx
        )
    ...
    def calculate_probability_ratio(self, old_policy_probs, states, episode):
        states = states.tolist()
        new_probs = []
        critic_value = []
        for s, e in zip(states, episode):
        ...
        new_probs = torch.stack([torch.stack(p) for p in new_probs]).to(
            self.actor1.device
        )
        ...
        for _ in range(self.n_epochs):
            ...
            for batch in batches:
                prob_ratio, critic_value = self.calculate_probability_ratio(
                    old_prob_arr[batch], state_arr[batch], episodes[batch]
                )
